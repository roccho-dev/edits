...
func collectBuffer(text,prefix string,limit int) []Candidate { freq := map[string]int{}; for _, w := range wordRE.FindAllString(text,-1) { freq[w]++ }; lower := strings.ToLower(prefix); out := []Candidate{}; for w,n := range freq { if w == prefix || !strings.HasPrefix(strings.ToLower(w), lower) { continue }; out = append(out, Candidate{Word:w, Source:"buffer", Rank:2000+n}) }; sort.Slice(out, func(i,j int) bool { if out[i].Rank != out[j].Rank { return out[i].Rank > out[j].Rank }; return out[i].Word < out[j].Word }); if limit > 0 && len(out) > limit { return out[:limit] }; return out }
