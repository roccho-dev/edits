...
func TestAcceptedHistoryRecallE2E(t *testing.T) {
	...
	const materialized = "@direct.open\npath=recent-safe.jsonl"
	...
	eventsPath := filepath.Join(filepath.Dir(acceptedPath), "events", "events.jsonl")
	eventsBefore, eventsExistedBefore := readOptionalFile(t, eventsPath)
	...
	captureReady := os.Getenv("HQ_PTY_CAPTURE_READY")
	captureDone := os.Getenv("HQ_PTY_CAPTURE_DONE")
	if (captureReady == "") != (captureDone == "") {
		t.Fatal("HQ_PTY_CAPTURE_READY and HQ_PTY_CAPTURE_DONE must be set together")
	}
	inputReady := os.Getenv("HQ_PTY_INPUT_READY")
	inputDone := os.Getenv("HQ_PTY_INPUT_DONE")
	if (inputReady == "") != (inputDone == "") {
		t.Fatal("HQ_PTY_INPUT_READY and HQ_PTY_INPUT_DONE must be set together")
	}
	scriptPath := writeHistoryPopupScript(t, root, requireVim9LSP(t), hqBin, bufferPath, proofPath, captureReady, captureDone, inputReady, inputDone)
	...
	if err := cmd.Run(); err != nil {
		...
		t.Fatalf("automatic accepted-history popup failed: %v\nproof=%s\nvim=%s", err, proof, log)
	}
	...
	if len(proof.Undo) != 1 || proof.Undo[0] != "@" {
		t.Fatalf("one undo did not restore the exact pre-selection query: %#v", proof.Undo)
	}
	...
	eventsAfter, eventsExistAfter := readOptionalFile(t, eventsPath)
	if eventsExistedBefore != eventsExistAfter || !bytes.Equal(eventsBefore, eventsAfter) {
		t.Fatalf("completion or selection changed events: before_exists=%t after_exists=%t before=%q after=%q", eventsExistedBefore, eventsExistAfter, eventsBefore, eventsAfter)
	}
...
func enableHistoryPolicy(t *testing.T, path string) {
	...
	before := `"name":"path","type":"path","required":true,"description":`
	after := `"name":"path","type":"path","required":true,"history_policy":"search","description":`
...
func writeHistoryPopupScript(t *testing.T, root, lspRoot, hqBin, bufferPath, proofPath, captureReady, captureDone, inputReady, inputDone string) string {
	t.Helper()
	q := func(value string) string { b, _ := json.Marshal(filepath.ToSlash(value)); return string(b) }
	expected, _ := json.Marshal([]string{"@direct.open", "path=recent-safe.jsonl"})
	lines := []string{
		...
		"let g:hq_history_capture_ready = " + q(captureReady),
		"let g:hq_history_capture_done = " + q(captureDone),
		"let g:hq_history_capture_deadline = 0.0",
		"let g:hq_history_input_ready = " + q(inputReady),
		"let g:hq_history_input_done = " + q(inputDone),
		"let g:hq_history_input_deadline = 0.0",
		...
		"function! HqHistoryCaptureWait(timer) abort",
		"  if empty(g:hq_history_capture_ready) || filereadable(g:hq_history_capture_done)",
		"    call feedkeys(\"\\<C-Y>\\<Esc>\", 'n')", "    call timer_start(100, function('HqHistoryVerify'))", "    return",
		"  endif",
		"  if reltimefloat(reltime()) >= g:hq_history_capture_deadline | call HqHistoryFinish('failed', 'PTY capture observer timeout') | return | endif",
		"  call timer_start(20, function('HqHistoryCaptureWait'))", "endfunction",
		...
		"  if !empty(g:hq_history_capture_ready)",
		"    let g:hq_history_capture_deadline = reltimefloat(reltime()) + 30.0",
		"    if writefile(['ready'], g:hq_history_capture_ready) != 0 | call HqHistoryFinish('failed', 'PTY capture ready write failed') | return | endif",
		"  endif",
		"  call HqHistoryCaptureWait(0)", "endfunction",
		...
		"function! HqHistoryInputWait(timer) abort",
		"  if filereadable(g:hq_history_input_done)",
		"    let g:hq_history_deadline = reltimefloat(reltime()) + 10.0", "    call timer_start(20, function('HqHistoryPoll'))", "    return",
		"  endif",
		"  if reltimefloat(reltime()) >= g:hq_history_input_deadline | call HqHistoryFinish('failed', 'PTY input timeout') | return | endif",
		"  call timer_start(20, function('HqHistoryInputWait'))", "endfunction",
		"HqStart",
		...
		"call cursor(1, 1)",
		"if empty(g:hq_history_input_ready)",
		"  call feedkeys('i@', 'n')", "  call timer_start(20, function('HqHistoryPoll'))",
		"else",
		"  let g:hq_history_input_deadline = reltimefloat(reltime()) + 30.0",
		"  if writefile(['ready'], g:hq_history_input_ready) != 0 | call HqHistoryFinish('failed', 'PTY input ready write failed') | else | call HqHistoryInputWait(0) | endif",
		"endif",
	}
...
func writeHistoryConformanceArtifact(t *testing.T, proof any) {
	...
	body, err := json.MarshalIndent(map[string]any{
		...
		"operation":        "type @ -> yegappan/lsp automatic native completion",
		"completionWrites": 0, "eventWrites": 0, "acceptedRows": 1,
		"undoBaseline": "@", "explicitSeedRows": 1, "proof": proof,
	}, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
