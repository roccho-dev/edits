...
V11=b8ab767c895972ce55ebbc6f1e55d61e9169626b
SOURCE_BRANCH=ux/focused-e2e-merge-source-v12-20260823
FINAL_BRANCH=ux/focused-e2e-merge-final-v12-20260823
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v12}

rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"/{source,product,e2e,runtime}

test "$(git rev-parse HEAD)" = "$V11"
...
if s.count(old) != 1:
    raise SystemExit(f'combined selection/accept matches={s.count(old)}')
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:02Z GIT_COMMITTER_DATE=2026-08-23T00:00:02Z
source=$(printf '%s\n' 'test(hq-vim): separate popup selection from acceptance' | git commit-tree "$tree" -p "$BASE")
test "$(git rev-parse "$source^")" = "$BASE"
printf 'base=%s\nv11=%s\nsource=%s\ntree=%s\n' "$BASE" "$V11" "$source" "$tree" | tee "$EVIDENCE/source/identity.txt"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:03Z GIT_COMMITTER_DATE=2026-08-23T00:00:03Z
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v12.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v12.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v12" PROOF_RUN_SUFFIX=v12 "$RUNNER_TEMP/runtime-proof-v12.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/3",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,popupInteraction:"select then accept",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V12_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
