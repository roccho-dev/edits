...
V12=1d7bf1c19f115bfad3f257b21f09f0d6bb31608a
SOURCE_BRANCH=ux/focused-e2e-merge-source-v13-20260823
FINAL_BRANCH=ux/focused-e2e-merge-final-v13-20260823
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v13}

rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"/{source,product,e2e,runtime}

test "$(git rev-parse HEAD)" = "$V12"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:04Z GIT_COMMITTER_DATE=2026-08-23T00:00:04Z
source=$(printf '%s\n' 'test(hq-vim): delay popup acceptance without selected-state polling' | git commit-tree "$tree" -p "$BASE")
test "$(git rev-parse "$source^")" = "$BASE"
printf 'base=%s\nv12=%s\nsource=%s\ntree=%s\n' "$BASE" "$V12" "$source" "$tree" | tee "$EVIDENCE/source/identity.txt"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:05Z GIT_COMMITTER_DATE=2026-08-23T00:00:05Z
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v13.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v13.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v13" PROOF_RUN_SUFFIX=v13 "$RUNNER_TEMP/runtime-proof-v13.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/4",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,popupInteraction:"select; timer; accept",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V13_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
