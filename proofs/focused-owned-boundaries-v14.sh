...
V13=b1e1c20dd1f89b995852fd675d55b8dc86a1894b
SOURCE_BRANCH=ux/focused-e2e-merge-source-v14-20260823
FINAL_BRANCH=ux/focused-e2e-merge-final-v14-20260823
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v14}

rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"/{source,product,e2e,runtime}

test "$(git rev-parse HEAD)" = "$V13"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:06Z GIT_COMMITTER_DATE=2026-08-23T00:00:06Z
source=$(printf '%s\n' 'fix(hq-vim): preserve undo guard across follow-on popup' | git commit-tree "$tree" -p "$BASE")
test "$(git rev-parse "$source^")" = "$BASE"
printf 'base=%s\nv13=%s\nsource=%s\ntree=%s\n' "$BASE" "$V13" "$source" "$tree" | tee "$EVIDENCE/source/identity.txt"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:07Z GIT_COMMITTER_DATE=2026-08-23T00:00:07Z
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v14.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v14.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v14" PROOF_RUN_SUFFIX=v14 "$RUNNER_TEMP/runtime-proof-v14.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/5",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,undoGuard:"deferred reset across follow-on popup",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V14_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
