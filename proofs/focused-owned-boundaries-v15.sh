...
V14=95167ec4da68e7e1f52ce1579296b9ea64aa497d
SOURCE_BRANCH=ux/focused-e2e-merge-source-v15-20260823
FINAL_BRANCH=ux/focused-e2e-merge-final-v15-20260823
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v15}

rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"/{source,product,e2e,runtime}

test "$(git rev-parse HEAD)" = "$V14"
test "$(git rev-parse HEAD^)" = "$BASE"

python3 - <<'PY'
from pathlib import Path
p = Path('packages/hq-vim/autoload/hq.vim')
s = p.read_text(encoding='utf-8')
old_acmd = '''    autocmd CompleteChanged <buffer> CompletionUndoBreak()\n    autocmd CompleteDone <buffer> CompletionUndoReset()\n'''
new_acmd = '''    autocmd CompleteChanged <buffer> CompletionUndoBreak()\n    autocmd CompleteDone <buffer> CompletionUndoReset()\n    autocmd InsertCharPre <buffer> CompletionUndoDisarm()\n    autocmd InsertLeave <buffer> CompletionUndoDisarm()\n'''
if s.count(old_acmd) != 1:
    raise SystemExit(f'completion autocmd block matches={s.count(old_acmd)}')
s = s.replace(old_acmd, new_acmd, 1)
old_reset = '''def CompletionUndoReset()\n  if get(b:, 'hq_completion_undo_restarting', false)\n    b:hq_completion_undo_restarting = false\n    timer_start(0, (_) => CompletionUndoResume())\n    return\n  endif\n  timer_start(0, (_) => CompletionUndoFinish())\nenddef\n\ndef CompletionUndoFinish()\n  if mode(1) =~# '^i' && pumvisible()\n    return\n  endif\n  b:hq_completion_undo_armed = false\nenddef\n\ndef CompletionUndoResume()\n'''
new_reset = '''def CompletionUndoReset()\n  if get(b:, 'hq_completion_undo_restarting', false)\n    b:hq_completion_undo_restarting = false\n    timer_start(0, (_) => CompletionUndoResume())\n    return\n  endif\nenddef\n\ndef CompletionUndoDisarm()\n  b:hq_completion_undo_armed = false\nenddef\n\ndef CompletionUndoResume()\n'''
if s.count(old_reset) != 1:
    raise SystemExit(f'deferred undo reset block matches={s.count(old_reset)}')
s = s.replace(old_reset, new_reset, 1)
p.write_text(s, encoding='utf-8')
PY

git diff --check
test "$(git diff --name-only "$V14" | tr -d '\n')" = packages/hq-vim/autoload/hq.vim
...
git add packages/hq-vim/autoload/hq.vim
tree=$(git write-tree)
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:08Z GIT_COMMITTER_DATE=2026-08-23T00:00:08Z
source=$(printf '%s\n' 'fix(hq-vim): scope completion undo guard to user input' | git commit-tree "$tree" -p "$BASE")
test "$(git rev-parse "$source^")" = "$BASE"
printf 'base=%s\nv14=%s\nsource=%s\ntree=%s\n' "$BASE" "$V14" "$source" "$tree" | tee "$EVIDENCE/source/identity.txt"
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:09Z GIT_COMMITTER_DATE=2026-08-23T00:00:09Z
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v15.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v15.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v15" PROOF_RUN_SUFFIX=v15 "$RUNNER_TEMP/runtime-proof-v15.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/6",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,undoGuard:"armed across automatic popup chain; disarm on user input or InsertLeave",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V15_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
