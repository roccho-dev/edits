...
SOURCE_BRANCH=ux/focused-e2e-merge-source-v17-20260823
FINAL_BRANCH=ux/focused-e2e-merge-final-v17-20260823
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v17}
...
old = '    feedkeys("\\<C-G>u", \'n\')\n'
new = '    feedkeys("\\<C-G>u", \'nt\')\n'
if s.count(old) != 1:
    raise SystemExit(f'undo boundary feedkeys matches={s.count(old)}')
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:12Z GIT_COMMITTER_DATE=2026-08-23T00:00:12Z
source=$(printf '%s\n' 'fix(hq-vim): queue typed completion undo boundary' | git commit-tree "$tree" -p "$BASE")
...
export GIT_AUTHOR_DATE=2026-08-23T00:00:13Z GIT_COMMITTER_DATE=2026-08-23T00:00:13Z
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v17.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v17.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v17" PROOF_RUN_SUFFIX=v17 "$RUNNER_TEMP/runtime-proof-v17.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/8",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,undoBoundary:"native Ctrl-G u queued as typed no-remap input before completion selection",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V17_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
