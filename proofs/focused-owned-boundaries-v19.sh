...
SOURCE_BRANCH=ux/focused-e2e-merge-source-v19-20260824
FINAL_BRANCH=ux/focused-e2e-merge-final-v19-20260824
EXPECTED_HQ=779a298f4efcff8df60205aaf973cb224388a82b
EVIDENCE=${EVIDENCE:-/tmp/focused-e2e-owned-boundaries-v19}
...
old_select = r'''call feedkeys(repeat("\<C-N>", l:steps), 'int')'''
new_select = r'''call feedkeys(repeat("\<C-N>", l:steps), 'nt')'''
if s.count(old_select) != 1:
    raise SystemExit(f'non-doc popup selection feedkeys matches={s.count(old_select)}')
s = s.replace(old_select, new_select, 1)
old_accept = '''\t\t"function! HqNativePopupAccept(timer) abort",\n\t\t`  call feedkeys("\\<C-Y>\\<Esc>", 'n')`,\n\t\t"  call timer_start(100, function('HqNativePopupVerify'))",\n\t\t"endfunction",'''
new_accept = '''\t\t"function! HqNativePopupAccept(timer) abort",\n\t\t"  let l:selected = get(complete_info(['selected']), 'selected', -1)",\n\t\t"  if l:selected != g:hq_native_expected_index",\n\t\t"    if reltimefloat(reltime()) >= g:hq_native_deadline",\n\t\t"      call HqNativePopupFinish('failed', 'native selection timeout before acceptance')",\n\t\t"      return",\n\t\t"    endif",\n\t\t"    call timer_start(20, function('HqNativePopupAccept'))",\n\t\t"    return",\n\t\t"  endif",\n\t\t`  call feedkeys("\\<C-Y>\\<Esc>", 'n')`,\n\t\t"  call timer_start(100, function('HqNativePopupVerify'))",\n\t\t"endfunction",'''
if s.count(old_accept) != 1:
    raise SystemExit(f'native accept function matches={s.count(old_accept)}')
p.write_text(s.replace(old_accept, new_accept, 1), encoding='utf-8')
PY
...
source=$(printf '%s\n' 'test(hq-vim): wait for native selection before acceptance' | git commit-tree "$tree" -p "$BASE")
...
cat proofs/vim-nix/run-proof.parts/*.sh > "$RUNNER_TEMP/runtime-proof-v19.sh"
chmod 700 "$RUNNER_TEMP/runtime-proof-v19.sh"
PROOF_ROOT="$product" PROOF_OUTPUT_DIR="$EVIDENCE/runtime" PROOF_RUNTIME_DIR="$RUNNER_TEMP/runtime-v19" PROOF_RUN_SUFFIX=v19 "$RUNNER_TEMP/runtime-proof-v19.sh" | tee "$EVIDENCE/runtime/stdout.log"
...
jq -n --arg base "$BASE" --arg source "$source" --arg sourceTree "$tree" --arg proof "$proof" --arg proofTree "$proof_tree" --arg product "$product" '{schema:"edits.focused-e2e-owned-boundaries/10",status:"PASS",base:$base,sourceCommit:$source,sourceTree:$sourceTree,proofCommit:$proof,proofTree:$proofTree,productStorePath:$product,editorBehaviorE2Es:8,runtimeLifecycleE2Es:1,popupSelection:"queued C-N; acceptance waits until native selected index is observable",resultKinds:["accepted","started","stdout","completed"],residualProcessCount:0,screenshotsPushed:false,e2eComposition:false}' > "$EVIDENCE/receipt.json"
(cd "$EVIDENCE" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS && sha256sum --check SHA256SUMS)
printf 'V19_PASS source=%s proof=%s product=%s\n' "$source" "$proof" "$product"
