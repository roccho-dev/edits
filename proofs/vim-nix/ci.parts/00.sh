...
printf '\n== Execute the complete proof from the Docker archive ==\n'
...
with tarfile.open(src, 'r:*') as tf:
    for m in tf.getmembers():
        ...
        rows.append((m, data))
target = next(i for i, (m, data) in enumerate(rows) if m.isfile() and m.name.lstrip('./').startswith('blobs/sha256/') and data)
m, data = rows[target]
b = bytearray(data); b[len(b)//2] ^= 1
